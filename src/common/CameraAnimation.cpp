...
void AutoZoomCameraAnimation::init() {
  ...
  if (m_zoomX > m_zoomY){
    ...
    m_fPreCameraStart.x = (m_motoGame->getLevelSrc()->RightLimit() + m_motoGame->getLevelSrc()->LeftLimit())/2;
    m_fPreCameraStart.y = m_motoGame->getLevelSrc()->TopLimit() - cameraStartHeight + PRESTART_ANIMATION_MARGIN_SIZE;
    m_fPreCameraFinal.x = (m_motoGame->getLevelSrc()->RightLimit() + m_motoGame->getLevelSrc()->LeftLimit())/2;
    m_fPreCameraFinal.y = m_motoGame->getLevelSrc()->BottomLimit() + cameraStartHeight - PRESTART_ANIMATION_MARGIN_SIZE;
    
    if ( fabs(m_fPreCameraStart.y - m_fPrePlayStartCamera.y) > fabs(m_fPreCameraFinal.y - m_fPrePlayStartCamera.y)) {
      float f;
    ...
    m_fPreCameraStart.y = (m_motoGame->getLevelSrc()->BottomLimit() + m_motoGame->getLevelSrc()->TopLimit())/2;
    m_fPreCameraFinal.x = m_motoGame->getLevelSrc()->LeftLimit() + cameraStartLeft - PRESTART_ANIMATION_MARGIN_SIZE;
    m_fPreCameraFinal.y = (m_motoGame->getLevelSrc()->BottomLimit() + m_motoGame->getLevelSrc()->TopLimit())/2;
    
    if ( fabs(m_fPreCameraStart.x - m_fPrePlayStartCamera.x) > fabs(m_fPreCameraFinal.x - m_fPrePlayStartCamera.x)) {
      float f;
...
bool AutoZoomCameraAnimation::step() {
  ...
  case 3:
    ...
    coeff = (GameApp::getXMTime() - m_startTimeUnzooming) / (INPLAY_ANIMATION_TIME);
    zx = coeff * (m_fAnimPlayStartCamera.x - m_fAnimPlayFinalCamera1.x);
    zy = coeff * (m_fAnimPlayStartCamera.y - m_fAnimPlayFinalCamera1.y);
    zz = coeff * (m_fAnimPlayStartZoom   - m_fAnimPlayFinalZoom);
...
void ZoomingCameraAnimation::init() {
  ...
  if (m_zoomX > m_zoomY){
    ...
    m_zoomU=m_zoomX;
    m_static_time = (m_motoGame->getLevelSrc()->TopLimit() - m_motoGame->getLevelSrc()->BottomLimit()) / (2.0/m_zoomU);
    ...
    if ( fabs(m_fPreCameraStart.y - initialPosition().y) > fabs(m_fPreCameraFinal.y - initialPosition().y)) {
      float f;
      f = m_fPreCameraFinal.y;
      m_fPreCameraFinal.y = m_fPreCameraStart.y;
      m_fPreCameraStart.y = f;
    ...
    if ( fabs(m_fPreCameraStart.x - initialPosition().x) > fabs(m_fPreCameraFinal.x - initialPosition().x)) {
      float f;
      f = m_fPreCameraFinal.x;
      m_fPreCameraFinal.x = m_fPreCameraStart.x;
      m_fPreCameraStart.x = f;
...
bool ZoomingCameraAnimation::step() {
  ...
  if(GameApp::getXMTime() > startTime() + m_static_time + PRESTART_ANIMATION_TIME) {
    return false;
  }
  if(GameApp::getXMTime() > startTime() + m_static_time){
    ...
    zy =  (PRESTART_ANIMATION_TIME + m_static_time - GameApp::getXMTime() + startTime())
      / (PRESTART_ANIMATION_TIME) 
      * (initialPosition().y - m_fPrePlayCameraLast.y);
    
    m_camera->setCameraPosition(initialPosition().x-zx, initialPosition().y-zy);
  } else {
    float zx,zy;
    ...
    zy = (m_static_time - GameApp::getXMTime() + startTime()) / (m_static_time) 
      * (m_fPreCameraStart.y - m_fPreCameraFinal.y);
    
    m_camera->setCameraPosition( m_fPreCameraStart.x  - zx, m_fPreCameraStart.y - zy);
