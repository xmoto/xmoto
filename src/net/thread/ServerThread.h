...
enum ServerP2Phase {
  SP2_PHASE_NONE,
  SP2_PHASE_WAIT_CLIENTS,
  SP2_PHASE_PLAYING
};

class NetSClient {
public:
  NetSClient(unsigned int i_id,
...
private:
  ...
  unsigned int m_numScene; // number of the scene in which the client plays (if
  // marked to play)
  unsigned int m_numPlayer; // number of the player in the scene which the
  // client plays (if marked to play)
...
public:
  ...
  NetSClient *getNetSClientByScenePlayer(unsigned int i_numScene,
...
private:
  ...
  int m_firstFrameSent; // send the first frame only one time before the game
  // starts
  ...
  ServerRules *m_rules;
  bool m_needToReloadRules; // rules are reloaded only when out of a round, not
  // immediatly when requested
  ...
  bool manageNetworkOnePacket(int i_timeout); // return true if there is
  // possibly something else to
  // manage on network

  // if i_execpt >= 0, send to all exept him
  void sendToAllClients(NetAction *i_netAction,
                        int i_src,
                        int i_subsrc,
                        int i_except = -1);
  void sendToAllClientsHavingMode(NetClientMode i_mode,
                                  NetAction *i_netAction,
                                  int i_src,
                                  int i_subsrc,
                                  int i_except = -1);
  void sendToAllClientsMarkedToPlay(NetAction *i_netAction,
                                    int i_src,
                                    int i_subsrc,
                                    int i_except = -1);
  void sendToAllClientsHavingProtocol(int i_protocol,
                                      NetAction *i_netAction_lt,
                                      NetAction *i_netAction_ge,
                                      int i_src,
                                      int i_subsrc,
                                      int i_except = -1);
  void sendToAllClientsHavingModeAndProtocol(NetClientMode i_mode,
                                             int i_protocol,
                                             NetAction *i_netAction_lt,
                                             NetAction *i_netAction_ge,
                                             int i_src,
                                             int i_subsrc,
                                             int i_except = -1);
  void sendToClient(NetAction *i_netAction,
                    unsigned int i,
                    int i_src,
                    int i_subsrc,
                    bool i_forceUdp = false);
