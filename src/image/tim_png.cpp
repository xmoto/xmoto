...
int tim_png_load(tim_session_t *pSession,tim_image_t **ppImage,tim_image_info_t *pInfo,
                 char *pcSource) {
	png_structp PngPtr;
	png_infop InfoPtr;
	png_infop EndInfo;
	int nChannels,nWidth,nHeight,nRowBytes,nBitDepth,nColorType,nNumberOfPasses,i;
	png_env_t Env;
	tim_image_t *pImg;
	tim_pixel_type_t pt;
	...
	if(Env.pvFile==NULL) return TIM_RV_ERR_NOT_FOUND;
	
	/* Init for reading */
	PngPtr=png_create_read_struct(PNG_LIBPNG_VER_STRING,(png_voidp)&Env,
  ...
  if(setjmp(png_jmpbuf(PngPtr))) {
    ...
    png_read_update_info(PngPtr,InfoPtr);
		...
		if(pImg==NULL)
			longjmp(png_jmpbuf(PngPtr),-1); /* Long jump! (yick!) I'm only using this because 
			                                   I enjoy hating myself :) */			
		*ppImage=pImg;
			                                	
		/* Load image */
		pRowPointers=(png_bytep *)tim_alloc(pSession,sizeof(png_bytep)*nHeight);
...
int tim_png_save(tim_image_t *pImage,char *pcTarget) {
	...
	PngPtr=png_create_write_struct(PNG_LIBPNG_VER_STRING,(png_voidp)&Env,
																 user_error_fn,user_warning_fn);
	if(!PngPtr) {
		tim_close(pSession,Env.pvFile);
		return TIM_RV_ERR_LIBRARY;	                              									
  ...
  png_set_compression_buffer_size(PngPtr,8192);
	...
	else longjmp(png_jmpbuf(PngPtr),-1);
	
	png_set_IHDR(PngPtr,InfoPtr,pImage->Info.nWidth,pImage->Info.nHeight,8,
							 nColorType,PNG_INTERLACE_NONE,PNG_COMPRESSION_TYPE_DEFAULT,
	...
	/* Write */						
	pRowPointers=(png_bytep *)tim_alloc(pSession,sizeof(png_bytep)*pImage->Info.nHeight);
	if(pRowPointers==NULL)
		longjmp(png_jmpbuf(PngPtr),-1);
...
int tim_add_png_support(tim_session_t *pSession) {
	...
	pFormat=tim_create_file_format(pSession,".png",tim_png_save,tim_png_load);
