...
typedef enum _tim_retval_t {
	...
	TIM_RV_ERR_INVALID_PARAM = -1,		/* Parametres parsed to function are not 
	                                     right */
	TIM_RV_ERR_COULD_NOT_PERFORM = -2,/* Could not perform the specified action */      
...
/* I/O modes */
typedef enum _tim_io_mode_t {
	TIM_IM_READ,TIM_IM_WRITE
} tim_io_mode_t;

/* Seek modes */
typedef enum _tim_seek_mode_t {
	TIM_SM_ABS,TIM_SM_REL,TIM_SM_END
} tim_seek_mode_t;

/* Function callback types */
typedef void *(*tim_callback_open_t)(char *pcWhere,tim_io_mode_t IOMode);
typedef void (*tim_callback_close_t)(void *pvHandle);
typedef int (*tim_callback_seek_t)(void *pvHandle,int nOffset,tim_seek_mode_t SeekMode);
...
typedef enum _tim_pixel_type_t {
	TIM_PT_RGB24,TIM_PT_RGBA32,TIM_PT_UNKNOWN
} tim_pixel_type_t;
...
/* Function callback types */
typedef int (*tim_callback_load_t)(struct _tim_session_t *pSession,tim_image_t **ppImage,
