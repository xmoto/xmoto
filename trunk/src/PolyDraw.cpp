...
TODO: assembler. :)  I simply don't think this code is fast enough to be of 
      practical use.
=============================================================================*/
  void PolyDraw::render(int *pnScreenVerts, int *pnTexVerts,
			int nNumCorners, SDL_Surface * pTexture) {
    /* If no buffers, abort */
	...
	_RenderHLine(y, &m_pRightEdge[y], &m_pLeftEdge[y], pTexture, nWSq,
		     nHSq, nPitchSq);
      } else {
	_RenderHLine(y, &m_pLeftEdge[y], &m_pRightEdge[y], pTexture, nWSq,
		     nHSq, nPitchSq);
      }
...
=============================================================================*/
  ...
  void PolyDraw::_LerpEdge(int y1, int y2, EdgeValue * pEdgeBuf,
			   int x1, int x2, int u1, int u2, int v1,
			   int v2) {
    int nYRange = y2 - y1;
  ...
  void PolyDraw::_RenderHLine(int y, EdgeValue * pLeft, EdgeValue * pRight,
			      SDL_Surface * pTexture, int nWSq, int nHSq,
			      int nPitchSq) {
    /* Determine pointer to beginning of line in target buffer */
    char *pc = &((char *)m_pBuf->pixels)[y * m_pBuf->pitch + MAX(0,
								 pLeft->
								 x) *
					 m_pBuf->format->BytesPerPixel];
	...
	case 1:
	  SDL_GetRGBA(((Uint8 *) pcT)[k], pTexture->format, &r, &g, &b,
		      &a);
	  break;
	case 2:
	  SDL_GetRGBA(((Uint16 *) pcT)[k], pTexture->format, &r, &g, &b,
		      &a);
	  break;
	case 3:		//bull??
	  printf("BULLLLLLLL\n");
	  ...
	  break;
	default:
	  SDL_GetRGBA(((Uint32 *) pcT)[k], pTexture->format, &r, &g, &b,
		      &a);
	}
