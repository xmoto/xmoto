...
void xmDatabase::fixStatsProfilesLevelsNbCompleted() {
  ...
  v_result = readDB("SELECT a.id_level, a.id_profile, count(*) AS nb "
  ...
  for(unsigned int i=0; i<nrow; i++) {
    ...
    simpleSql("UPDATE stats_profiles_levels SET nbCompleted=" + v_nb + " WHERE id_profile=\"" + protectString(v_profile) + "\" AND id_level=\"" + protectString(v_id_level) + "\"");
  }
