...
void *tim_stdio_open(char *pcWhere,tim_io_mode_t IOMode) {
	...
	switch(IOMode) {
		case TIM_IM_READ: pcIOMode="rb"; break;
		case TIM_IM_WRITE: pcIOMode="wb"; break;
	}
	
	/* No prober I/O mode? */
	if(pcIOMode==NULL) return NULL;
...
int tim_stdio_seek(void *pvHandle,int nOffset,tim_seek_mode_t SeekMode) {
	...
	switch(SeekMode) {
		case TIM_SM_ABS: nSeekMode=SEEK_SET; break; /* Absolute seeking */
		case TIM_SM_REL: nSeekMode=SEEK_CUR; break; /* Seek relatively to current pos */
		case TIM_SM_END: nSeekMode=SEEK_END; break; /* Seek relatively to end */
	}
	
	/* No prober SeekMode? */
	if(nSeekMode<0) return TIM_RV_ERR_INVALID_PARAM;
	...
	if(nCurPos<0) return TIM_RV_ERR_COULD_NOT_PERFORM;
...
tim_io_t g_IO_Stdio={
	tim_stdio_open,tim_stdio_close,tim_stdio_seek,tim_stdio_read,tim_stdio_write,
	tim_stdio_eof
};
