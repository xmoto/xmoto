...
UIButtonDrawn::UIButtonDrawn(UIWindow *pParent,
  ...
  : UIButton(pParent, x, y, Caption, nWidth, nHeight) {
    ...
    v_sprite = ThemeManager::instance()->getTheme(XMSession::instance()->theme())->getSprite(SPRITE_TYPE_UI, i_spritePressed);
    if(v_sprite != NULL) {
      m_texturePressed = v_sprite->getTexture();
    }

    v_sprite = ThemeManager::instance()->getTheme(XMSession::instance()->theme())->getSprite(SPRITE_TYPE_UI, i_spriteUnpressed);
    if(v_sprite != NULL) {
      m_textureUnpressed = v_sprite->getTexture();
    }

    v_sprite = ThemeManager::instance()->getTheme(XMSession::instance()->theme())->getSprite(SPRITE_TYPE_UI, i_spriteHover);
    if(v_sprite != NULL) {
