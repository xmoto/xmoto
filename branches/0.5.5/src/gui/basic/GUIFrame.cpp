...
#include "../../Game.h"
  ...
  UIFrame::UIFrame(UIWindow *pParent,
    ...
    Sprite *pSprite;
    pSprite = ThemeManager::instance()->getTheme(XMSession::instance()->theme())->getSprite(SPRITE_TYPE_UI, "MenuTL");
    if(pSprite != NULL) {
      m_pMenuTL = pSprite->getTexture(false,true, FM_NEAREST);
    }

    pSprite = ThemeManager::instance()->getTheme(XMSession::instance()->theme())->getSprite(SPRITE_TYPE_UI, "MenuTR");
    if(pSprite != NULL) {
      m_pMenuTR = pSprite->getTexture(false,true, FM_NEAREST);
    }

    pSprite = ThemeManager::instance()->getTheme(XMSession::instance()->theme())->getSprite(SPRITE_TYPE_UI, "MenuBL");
    if(pSprite != NULL) {
      m_pMenuBL = pSprite->getTexture(false,true, FM_NEAREST);
    }

    pSprite = ThemeManager::instance()->getTheme(XMSession::instance()->theme())->getSprite(SPRITE_TYPE_UI, "MenuBR");
    if(pSprite != NULL) {
