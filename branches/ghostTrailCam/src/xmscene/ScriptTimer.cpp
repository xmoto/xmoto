...
ScriptTimer::ScriptTimer(const std::string& i_name,int i_delay,int i_loops, LuaLibGame *i_Script, int i_GameTime){
	m_TimerName = i_name;
...
void ScriptTimer::UpdateTimer(int i_GameTime){
	if(m_isRunning==true){ //only run if not paused
		if(i_GameTime>m_TimeOfLastCall+m_TimeBetweenCalls){ //time to call again
			m_TimeOfLastCall = i_GameTime; //update the timer
			m_Script->scriptCallTblVoid(m_TimerName, "Tick", m_Numbr_Of_Calls);//call the function as needed
