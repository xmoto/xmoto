...
namespace vapp {
  ...
  class BezierCurve {
    public:
      ...
      /* Data interface */
      Vector3f &getP(int n) {
        if(n<0 || n>3) throw Exception("(1) cubic bezier point index out of bounds");
        return m_P[n];
      }
      void setP(int n,const Vector3f &p) {
        if(n<0 || n>3) throw Exception("(2) cubic bezier point index out of bounds");        
        m_P[n] = p;
      }
